...
void foo(const Animal& arg) {
	cout << "\n\033[31mCalling virtual function from inside foo() (arg passed as const Animal REFERENCE)\n\x1b\x5bm" ANSI_RST;
	arg.makeSound();
	...
	repr_init();
{
	...
	cout << "\n\033[31mPassing WOOFY to foo(Animal) (by value!)" ANSI_RST "\n";
	foo(*woofy);
...
}
{
	...
	Animal *animals[n_animals] = {new Cat(), new Dog(), new Cat(), new Dog()};
